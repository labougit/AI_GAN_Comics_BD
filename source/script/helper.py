...
def show_active_img_and_save_denoise(name,img,path,raw):
    mat = img.astype(np.float)
    mat = - mat + 1
    mat = mat * 255.0
    mat[mat < 0] = 0
    ...
    #cv2.imshow(name,mat)
    #print("Tableau noir et blanc: ", cv2.cvtColor(mat, cv2.COLOR_GRAY2RGB).shape)
    #print("Tableau couleur: ", raw.shape)

    new_mat = np.concatenate((raw, cv2.cvtColor(mat, cv2.COLOR_GRAY2RGB)), axis=1)
    cv2.imwrite(path,new_mat)
    return
