...
# Changement nouvel fonction CTOS Opencv
def C2S(img):
...
class PairImageFolder(datasets.ImageFolder):   
    ...
    def __getitem__(self, index):
        ...
        #sketch_copy_img = copy.deepcopy(img_tmp) # copie profonde
        ...
        ####################
        # Changement img_edge par opencv
        grey_img = cv2.cvtColor(img_tmp,cv2.COLOR_BGR2GRAY)
        grey_img = cv2.merge([grey_img, grey_img, grey_img])
        invert = cv2.bitwise_not(grey_img)
        blur = cv2.GaussianBlur(invert, (21,21),0)
        invertedblur = cv2.bitwise_not(blur)
        sketch = cv2.divide(grey_img, invertedblur, scale=256.0)
        ...
        #with torch.no_grad():
            ...
            #img_egde = img_edge.unsqueeze(0).to(self.device)
